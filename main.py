...
async def recognize_image(payload: dict = Body(...)):
    ...
    try:
        ...
        matching_prompt_parts = [
            "첫 번째 이미지는 사용자가 찍은 사진이고, 그 뒤의 이미지들은 데이터베이스의 작품 목록입니다. 사용자의 사진과 가장 일치하는 작품의 파일명을 `[MATCH: 파일명]` 형식으로만 응답해주세요. 다른 설명은 절대 추가하지 마세요.",
            user_image,
        ]
        ...
        gemini_response = await MODEL.generate_content_async(matching_prompt_parts)
        match = re.search(r"\[MATCH:\s*(page_\d+_img_\d+\.png)\]", gemini_response.text)
