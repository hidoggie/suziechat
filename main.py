...
def startup_event():
    ...
    try:
        ...
        # ✨ 1. PDF 처리 로직으로 교체 ✨
        IMAGES_DIR.mkdir(exist_ok=True) # 이미지 저장 폴더 생성
        ...
        for page_num, page in enumerate(doc):
            ...
            # --- ✨ Pillow를 사용한 이미지 변환 로직 (핵심 수정) ✨ ---
                try:
                    ...
                    print(f"경고: 이미지 변환/저장 실패 (페이지 {page_num}, 이미지 {img_index}): {img_conv_e}")
                # --- ✨ 여기까지 수정 ---
                ...
                print(f"✅ PDF 처리 완료: 총 {len(doc)} 페이지, {sum(len(p['images']) for p in PDF_CONTENT)}개 이미지 추출 및 PNG 변환 완료")
    ...
    try:
        while True:
            ...
            if user_text:
               ...
               # --- ✨ 1. 질문과 관련된 페이지 찾기 (간단한 RAG) ✨ ---
                # 사용자 질문의 키워드가 포함된 텍스트가 있는 페이지만 필터링
                keywords = user_text.split()
                relevant_pages = [
                    page for page in PDF_CONTENT 
                    if any(keyword.lower() in page["text"].lower() for keyword in keywords)
                ]
                if not relevant_pages: # 관련된 페이지가 없으면 전체 텍스트를 컨텍스트로 사용
                    relevant_pages = PDF_CONTENT

                # --- ✨ 2. Gemini에게 전달할 프롬프트 재구성 (텍스트 + 이미지) ✨ ---
                prompt_parts = []
                # 시스템 명령어는 모델 자체에 설정되어 있으므로, 여기서는 질문과 컨텍스트만 전달
                prompt_parts.append("아래 제공된 텍스트와 이미지를 참고하여 다음 질문에 답변해줘.")
                ...
                for page in relevant_pages:
                    ...
                    for img_file in page["images"]:
                        ...
                        if img_path.exists():
                            try:
                                print(f"🖼️ Gemini에게 이미지 전달: {img_file}")
                ...
                image_tag_match = re.search(r"\[IMAGE:\s*(.*?)\]", ai_text_raw)
