# main.py (0728 오후 3시 05분 최종 완성 및 통합 버전)
    ...
    try:
        while True:
            ...
            if user_text:
                ...
                # --- ✨✨✨ 2. 컨텍스트를 'PDF 전체'로 설정 (핵심 수정) ✨✨✨
                # 키워드로 일부 페이지만 검색하던 로직을 삭제합니다.
                full_context_text = "\n\n".join([page['text'] for page in PDF_CONTENT])

                # --- 3. AI에게 이미지 표시 여부를 스스로 결정하도록 지시 ---
                # 컨텍스트 이미지 목록은 이제 필요 없으므로 프롬프트에서 제외합니다.
                prompt = f"""
                당신은 전문 도슨트입니다. 아래 '전체 컨텍스트'를 바탕으로 사용자의 '질문'에 답변해주세요.

                [매우 중요한 규칙]
                1. 답변은 항상 '전체 컨텍스트'에 있는 내용만을 근거로 생성해야 합니다.
                2. 만약 사용자가 "어떻게 생겼어", "보여줘", "그림", "사진" 등 시각적인 정보를 명시적으로 요구하고, '전체 컨텍스트' 내용으로 답변이 가능하다면, 답변의 맨 끝에 `[SHOW_IMAGE]` 태그를 추가해주세요.
                3. 일반적인 질문에는 절대 `[SHOW_IMAGE]` 태그를 붙이면 안 됩니다.
                ...
                # --- 4. 응답 파싱 및 이미지 전송 (이미지 찾기 로직 추가) ---
                image_tag_match = re.search(r"\[SHOW_IMAGE\]", ai_text_raw)
                ai_text = re.sub(r"\[SHOW_IMAGE\]", "", ai_text_raw).strip()
                await websocket.send_json({"type": "ai_text", "data": ai_text})

                if image_tag_match:
                    # AI가 이미지를 보여주라고 판단했으면, 질문 내용으로 가장 관련있는 이미지를 '다시' 찾아서 전송
                    ...
                    if best_match["page_data"] and best_match["page_data"]["images"]:
                        image_filename = best_match["page_data"]["images"][0] # 가장 첫 번째 이미지를 대표 이미지로 전송
...
async def recognize_image(payload: dict = Body(...)):
    ...
    try:
        ...
        describe_prompt = ["이 이미지에 보이는 건축물이나 사물의 가장 구체적인 전체 이름(proper noun)을 포함하여 상세하게 묘사해줘.", user_image]
        print("🤖 Gemini에게 이미지 상세 묘사 요청...")

        describe_response = await MODEL.generate_content_async(describe_prompt)
        description_text = describe_response.text.strip()
        print(f"✅ 이미지 묘사 생성: '{description_text[:50]}...'")

        keywords = list(set(re.findall(r'[\w가-힣]{2,}', description_text))) # 2글자 이상의 단어 추출
        print(f"🔍 추출된 검색 키워드: {keywords[:5]}")

        best_match = {"score": 0, "page_data": None}
        for page in PDF_CONTENT:
            score = 0
            # 키워드가 페이지 텍스트에 얼마나 포함되는지 점수 계산
