...
from fastapi.responses import FileResponse # ✨ 2. FileResponse 임포트
from fastapi.staticfiles import StaticFiles # ✨ 3. StaticFiles 임포트
...
# ✨✨✨ 4. 새로운 파일 서빙 로직 ✨✨✨
# 현재 파일(main.py)의 위치를 기준으로 기본 디렉터리를 설정합니다.
BASE_DIR = Path(__file__).resolve().parent

# 루트 경로("/")로 접속하면 'static' 폴더의 index.html을 보여줍니다.
@app.get("/", response_class=FileResponse)
async def read_index():
    # FileResponse를 사용하면 더 효율적이고 안전합니다.
    return FileResponse(BASE_DIR / "static" / "index.html")

# '/static' 경로를 'static' 폴더에 연결(mount)합니다.
# 이렇게 하면 나중에 index.html이 CSS나 JS 파일을 참조할 때도 문제가 없습니다.
app.mount("/static", StaticFiles(directory=BASE_DIR / "static"), name="static")
# ✨✨✨ 여기까지가 새로운 파일 서빙 로직입니다 ✨✨✨
