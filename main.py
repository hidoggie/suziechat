...
async def get_pdf_content():
    if not PDF_CONTENT: return JSONResponse(status_code=500, content={"error": "PDF content not loaded or failed to load"})
    content_map = {}
    for page in PDF_CONTENT:
        for image_file in page["images"]:
            content_map[image_file] = page["text"]
    
    return JSONResponse(content=content_map)
