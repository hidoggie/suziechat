    ...
    try:
        while True:
            ...
            if user_text:
               ...
               # ✨ 3. Gemini에게 텍스트와 이미지를 함께 전달하는 로직 ✨
                # 간단한 예시로, 전체 텍스트와 모든 이미지를 컨텍스트로 사용
                # (고급: RAG를 사용해 관련 있는 페이지만 선별해야 함)
                full_text_context = "\n".join([p["text"] for p in PDF_CONTENT])
                prompt_parts = [f"""'제공된 내용'과 '이미지'들을 바탕으로 다음 질문에 답변해줘.
                --- 제공된 내용 ---
                {full_text_context}
                --- 질문 ---
                {user_text}
                ...
                # 이미지 파일들을 프롬프트에 추가 (이 부분은 gemini-1.5-pro에서 더 잘 작동합니다)
