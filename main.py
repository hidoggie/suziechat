...
async def websocket_endpoint(websocket: WebSocket):
 ...
 #               full_context_text = "\n\n".join([page['text'] for page in PDF_CONTENT])
         ...
         #---       context_text = "\n\n".join([p['text'] for p in PDF_CONTENT]) # 기본은 전체 컨텍스트
                ...
                prompt = f"""당신은 전문 도슨트입니다. 아래 '컨텍스트'를 바탕으로 사용자의 '질문'에 답변해주세요.
                [규칙] 1. 답변은 '컨텍스트' 내용만을 근거로 생성해야 합니다. 
                2. 사용자가 "보여줘", "그림", "사진" 등 시각 정보를 명시적으로 요구하는 경우에만, 답변의 맨 끝에 `[SHOW_IMAGE]` 태그를 추가해주세요. 
                3. 일반 질문에는 절대 `[SHOW_IMAGE]` 태그를 붙이면 안 됩니다.
             ...
             #--   await websocket.send_json({"type": "ai_text", "data": ai_text})
                ...
                # ✨✨✨ 3. 응답에서 태그를 '제거'하여 깨끗한 텍스트만 전송 (핵심 수정) ✨✨✨
                image_tag_match = re.search(r"\[SHOW_IMAGE\]", ai_text_raw)
                ai_text = re.sub(r"\[SHOW_IMAGE\]", "", ai_text_raw).strip()
                await websocket.send_json({"type": "ai_text", "data": ai_text})

                # --- 4. 태그가 있었을 경우, 찾았던 컨텍스트의 이미지를 전송 ---
                if image_tag_match and context_images:
                    image_filename = context_images[0] # 페이지의 첫 번째 이미지를 대표로 전송
