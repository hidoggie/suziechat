...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        for page_num in range(len(doc)):
            ...
            page_data = {"page": page_num, "text": page_text}
            image_files = []
    ...
    # ✨ PDF 이미지들을 메모리에 로딩하는 부분 추가
    for page in PDF_CONTENT:
        for img_file in page["images"]:
            img_path = IMAGES_DIR / img_file
            if img_path.exists():
                PDF_IMAGES_IN_MEMORY[img_file] = Image.open(img_path)
    print(f"✅ 메모리에 {len(PDF_IMAGES_IN_MEMORY)}개의 PDF 이미지를 로드했습니다.")
...
async def recognize_image(payload: dict = Body(...)):
    ...
    try:
        ...
        # --- 1단계: AI에게 이미지 매칭만 요청 ---
        matching_prompt_parts = [
            "You are an image recognition expert. The first image is from a user. The following images are from a database. Find the single best match for the user's image from the database images. You MUST respond ONLY with the matched image's filename in the format `[MATCH: filename.png]`. Do not add any other text.",
            user_image,
        ]
        for filename, img_obj in PDF_IMAGES_IN_MEMORY.items():
            matching_prompt_parts.append(filename)
            matching_prompt_parts.append(img_obj)

        print("🤖 Gemini에게 이미지 매칭 요청...")
        gemini_response_match = await MODEL.generate_content_async(matching_prompt_parts)    
        match = re.search(r"\[MATCH:\s*(page_\d+_img_\d+\.png)\]", gemini_response_match.text)
        
        if not match:
            print("❌ 일치하는 이미지를 찾지 못함.")
            return {"status": "no_match", "description": "죄송합니다, 일치하는 전시물을 찾을 수 없습니다."}

        # --- 2단계: 서버가 매칭된 이미지의 PDF 텍스트 검색 ---
        matched_filename = match.group(1).strip()
        context_text = ""
        for page in PDF_CONTENT:
            if matched_filename in page["images"]:
                context_text = page["text"]
                break
        
        if not context_text:
            return {"status": "no_match", "description": "오류: 이미지는 찾았지만, 연관된 텍스트를 찾을 수 없습니다."}

          # --- ✨✨✨ 3단계: AI에게 보내는 '텍스트 요약' 프롬프트 수정 (핵심) ✨✨✨
        summarize_prompt = f"""
        당신은 전문 박물관 도슨트입니다.
        아래에는 박물관 카탈로그의 전체 페이지 내용과, 그 페이지에 있는 특정 이미지의 파일명이 주어집니다.

        당신의 임무는 다음과 같습니다:
        1. '전체 페이지 내용' 중에서, '특정 이미지 파일명'과 관련된 부분만을 정확히 찾아내세요.
        2. 찾아낸 그 부분만을 바탕으로, 실제 관람객에게 설명하듯이 생생하고 흥미로운 해설을 2000자 이내로 간결하게 생성해주세요.
        3. [매우 중요] "안녕하세요", "여러분", "환영합니다"와 같은 서두의 인사말은 절대 사용하지 마세요. "이번 작품은..." 또는 "이것은..." 과 같이 바로 설명으로 시작해야 합니다.
        4. 주어진 '전체 페이지 내용'에 없는 정보는 절대 지어내서는 안 됩니다.
