    ...
    try:
        while True:
            ...
            if user_text:
                ...
                keywords = re.findall(r'[\w가-힣]{2,}', user_text) # 2글자 이상 단어 추출
                ...
                if best_match["page_data"]:
                    ...
                    print(f"✅ 컨텍스트 찾음: 페이지 {best_match['page_data']['page']}")
                else: # 일치하는 페이지가 없으면 전체 텍스트 사용
                    context_text = "\n".join([p["text"] for p in PDF_CONTENT])

                # --- ✨ 2. AI에게 이미지 표시 여부를 스스로 결정하도록 지시 ---
                prompt = f"""
                당신은 전문 도슨트입니다. 아래 '컨텍스트'를 바탕으로 사용자의 '질문'에 답변해주세요.

                [매우 중요한 규칙]
                1. 답변은 항상 '컨텍스트'에 있는 내용만을 근거로 생성해야 합니다.
                2. 만약 사용자가 "어떻게 생겼어", "보여줘", "그림", "사진" 등 시각적인 정보를 명시적으로 요구하는 경우에만, 답변의 맨 끝에 `[SHOW_IMAGE]` 태그를 추가해주세요.
                3. '컨텍스트 이미지 목록'이 있다면, `[SHOW_IMAGE]` 태그 뒤에 가장 관련 있는 이미지 파일명 하나를 골라 `[SHOW_IMAGE:{context_images[0]}]` 와 같이 붙여주세요.
                ...
                if image_tag_match:
                    ...
                    if image_filename in context_images: # 컨텍스트에 있는 이미지인지 재확인
