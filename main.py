...
async def recognize_image(payload: dict = Body(...)):
    ...
    try:
        ...
        # --- 3단계: AI에게 검색된 텍스트의 요약만 요청 ---
        summarize_prompt = f"""
        당신은 전문 박물관 도슨트입니다.
        아래 텍스트는 한 전시물에 대한 정보입니다. 이 텍스트의 핵심 내용만을 바탕으로, 실제 관람객에게 설명하듯이 생생하고 흥미로운 해설을 2000자 이내로 간결하게 생성해주세요.
        주어진 텍스트에 없는 내용을 절대 지어내거나, 이미지 파일명을 언급해서는 안 됩니다.
