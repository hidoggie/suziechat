...
async def recognize_image(payload: dict = Body(...)):
    ...
    try:
        # 1. 사용자가 보낸 이미지를 Pillow 이미지 객체로 변환
        user_image_bytes = base64.b64decode(user_image_b64.split(',')[1])
        user_image = Image.open(io.BytesIO(user_image_bytes))

        # 2. Gemini에게 보낼 프롬프트 구성 (사용자 사진 + DB의 모든 사진)
        prompt_parts = [
            "당신은 전시관의 AI 도슨트입니다. 첫 번째 이미지는 사용자가 찍은 사진이고, 그 뒤의 이미지들은 전시관의 작품 목록입니다.",
            "사용자의 사진과 가장 일치하는 작품을 작품 목록에서 찾아, 그 작품에 대한 해설을 2000자 이내로 간결하고 흥미롭게 생성해주세요.",
            "일치하는 작품을 찾았다면, 답변 맨 끝에 반드시 `[MATCH: 작품이미지파일명.png]` 태그를 붙여주세요.",
            "일치하는 작품을 찾지 못했다면, `[NO_MATCH]` 태그만 포함하여 응답해주세요.",
            "--- 사용자 사진 ---",
            user_image,
            "--- 작품 목록 ---"
        ]
        # 메모리에 로드된 작품 이미지들을 프롬프트에 추가
        for filename, img_obj in PDF_IMAGES_IN_MEMORY.items():
            prompt_parts.append(filename)
            prompt_parts.append(img_obj)

        # 3. Gemini 멀티모달 모델 호출
        print("🤖 Gemini에게 이미지 인식 및 해설 생성 요청...")
        gemini_response = await MODEL.generate_content_async(prompt_parts)
        
        # 4. Gemini 응답 파싱
        response_text = gemini_response.text
        match = re.search(r"\[MATCH:\s*(.*?)\]", response_text)
        
        if match:
            matched_filename = match.group(1).strip()
            description = re.sub(r"\[MATCH:\s*(.*?)\]", "", response_text).strip()
            print(f"✅ 일치하는 이미지 찾음: {matched_filename}")
            return {"status": "success", "description": description, "image_name": matched_filename}
        else:
            print("❌ 일치하는 이미지를 찾지 못함.")
            return {"status": "no_match", "description": "죄송합니다, 이 이미지와 일치하는 전시물을 데이터베이스에서 찾을 수 없습니다."}
